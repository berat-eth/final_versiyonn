...
function initializeApp() {
    // Login kontrolü: TENANT_API_KEY yoksa login sayfasına yönlendir
    try {
    ...
    // Açılışta otomatik sağlık taraması devre dışı (isteğe bağlı çalıştırılır)
    try {
...
async function changeAdminPassword() {
    try {
        ...
        if (!currentPassword || !newPassword) {
            showNotification('Lütfen mevcut ve yeni şifreyi girin', 'error');
            return;
        }
        ...
        showNotification('Şifre güncellenemedi: ' + e.message, 'error');
    }
}
...
function handleRestoreFile(ev) {
    ...
    reader.onload = async () => {
        try {
            ...
            if (!res.ok) throw new Error(`HTTP ${res.status}`);
            showNotification('Geri yükleme tamamlandı', 'success');
        } catch (e) {
            showNotification('Geri yükleme başarısız: ' + e.message, 'error');
        } finally { showLoading(false); }
    };
...
async function saveFtpConfig() {
    try {
        ...
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        showNotification('FTP ayarları kaydedildi', 'success');
    } catch (e) { showNotification('FTP ayarları kaydedilemedi: ' + e.message, 'error'); }
}
...
async function testFtpBackup() {
    try {
        ...
        showNotification('FTP test yedeği gönderildi', 'success');
    } catch (e) { showNotification('FTP test başarısız: ' + e.message, 'error'); }
}
    ...
    const errorHtml = `
        <div class="connection-error">
            ...
            <div class="error-details">
                ...
                <ul>
                    <li>Uzak sunucu çalışıyor mu? (api.zerodaysoftware.tr)</li>
                ...
                <div class="connection-info">
                    ...
                    <p>🌐 Uzak Sunucu: api.zerodaysoftware.tr</p>
...
1. Uzak Sunucu Kontrolü:
   - Alan adı: api.zerodaysoftware.tr
...
async function runBackendHealthAudit() {
    ...
    async function testEndpoint(ep) {
        ...
        try {
            ...
            let hint = '';
            if (res.status === 401 || res.status === 403) hint = ' (Yetkilendirme gerekli - X-API-Key eksik/yanlış)';
    ...
    resultsEl.innerHTML = `
        ...
        <div style="margin-top:10px">
            <small>Not: Yetkilendirme isteyen endpointler için 401/403 normaldir. <code>X-API-Key</code> hatalı/eksik ise Ayarlar > Tenant API Key alanından güncelleyin.</small>
        </div>
    `;
}
...
// Basit istek kuyruğu, önbellek ve tekrar çağrı azaltma (Rate limit devre dışı)
...
function enqueueRequest(task){
    // Rate limit/kuyruk devre dışı: görevi hemen çalıştır
    return Promise.resolve().then(task);
}

function processQueue(){
    // Kuyruk mekanizması devre dışı
}
...
async function apiRequest(endpoint, options = {}) {
    ...
    try {
        ...
        if (error && (error.status === 401 || error.status === 403)) {
            showNotification('Yetkilendirme hatası: X-API-Key geçersiz veya eksik. Lütfen Ayarlar > Tenant API Key alanına geçerli anahtarı girin.', 'error');
...
async function loadDashboardData(range) {
    try {
        ...
        // ikinci dalga: daha az kritik + bayi ve özel üretim
        ...
        // Bayi ve Özel Üretim dashboard kartları ve tablo
        try {
...
function initializeCharts(data) {
    ...
    if (typeof Chart === 'undefined') {
        ...
        showNotification('Grafik kütüphanesi yüklenemedi', 'error');
        return;
    }
...
function updateDashboardCustomProdTable(custom = []) {
    ...
    const rows = custom.slice(0,10).map(r => `
        <tr>
            ...
            <td>${typeof r.totalAmount === 'number' ? formatCurrency(r.totalAmount) + ' ₺' : '-'}</td>
...
async function exportDashboardPdf() {
    try {
        ...
        if (!el) { showNotification('Dashboard bulunamadı', 'error'); return; }
        const canvas = await html2canvas(el, { scale: 2, useCORS: true });
        const imgData = canvas.toDataURL('image/png');
        const { jsPDF } = window.jspdf || window.jspdf || {};
        if (!jsPDF) { showNotification('PDF kütüphanesi yüklenemedi', 'error'); return; }
        ...
        showNotification('PDF oluşturuldu', 'success');
    } catch (e) {
        showNotification('PDF oluşturulamadı: ' + (e.message || ''), 'error');
    } finally {
        showLoading(false);
    }
}
...
async function loadCampaigns() {
    try {
        ...
        const msg = (error && (error.status === 401 || error.status === 403))
            ? 'Yetkilendirme gerekiyor. Lütfen X-API-Key ayarlayın.'
