      ...
      <div className="bg-white rounded-2xl shadow-sm p-6">
        <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => toggleSort('balance')} className="px-3 py-2 border border-slate-200 rounded-lg hover:bg-slate-50 text-sm">
              Bakiye <ArrowUpDown className="inline w-4 h-4 ml-1" />
            </button>
            <button onClick={() => toggleSort('name')} className="px-3 py-2 border border-slate-200 rounded-lg hover:bg-slate-50 text-sm">
              İsim <ArrowUpDown className="inline w-4 h-4 ml-1" />
            </button>
            <button onClick={() => toggleSort('email')} className="px-3 py-2 border border-slate-200 rounded-lg hover:bg-slate-50 text-sm">
              Email <ArrowUpDown className="inline w-4 h-4 ml-1" />
            </button>
            <button onClick={exportCsv} className="px-3 py-2 bg-slate-900 text-white rounded-lg hover:opacity-90 text-sm flex items-center">
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between pt-2">
            <div className="text-sm text-slate-600">Toplam {sorted.length} cüzdan</div>
            <div className="flex items-center gap-2">
              <button disabled={page<=1} onClick={() => setPage(p => Math.max(1, p-1))} className="px-3 py-1.5 border border-slate-200 rounded-lg disabled:opacity-50">Önceki</button>
              <span className="text-sm text-slate-700">{page}/{totalPages}</span>
              <button disabled={page>=totalPages} onClick={() => setPage(p => Math.min(totalPages, p+1))} className="px-3 py-1.5 border border-slate-200 rounded-lg disabled:opacity-50">Sonraki</button>
              <select value={pageSize} onChange={(e)=>{ setPageSize(parseInt(e.target.value)); setPage(1) }} className="ml-2 px-2 py-1 border border-slate-200 rounded-lg text-sm">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowTxModal(false)}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-slate-200 flex items-center justify-between">
              <div>
                <h3 className="text-2xl font-bold text-slate-800">Hesap Hareketleri</h3>
            ...
            <div className="p-6">
              ...
              <div className="flex items-center justify-between gap-2 mb-3">
                <div className="flex items-center gap-2">
                  <select value={txTypeFilter} onChange={(e)=>{ setTxTypeFilter(e.target.value as any); setTxPage(1) }} className="px-3 py-2 border border-slate-300 rounded-lg text-sm">
                  ...
                  <div className="relative">
                    ...
                    <input value={txSearch} onChange={(e)=>{ setTxSearch(e.target.value); setTxPage(1) }} placeholder="İşlem ara..." className="pl-7 pr-3 py-2 border border-slate-300 rounded-lg text-sm" />
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={()=>{ setTxSortKey('date'); setTxSortDir(d=> d==='asc'?'desc':'asc') }} className="px-3 py-2 border border-slate-300 rounded-lg text-sm">Tarih</button>
                  <button onClick={()=>{ setTxSortKey('amount'); setTxSortDir(d=> d==='asc'?'desc':'asc') }} className="px-3 py-2 border border-slate-300 rounded-lg text-sm">Tutar</button>
                  <button onClick={()=>{ setTxSortKey('type'); setTxSortDir(d=> d==='asc'?'desc':'asc') }} className="px-3 py-2 border border-slate-300 rounded-lg text-sm">Tür</button>
                  <button onClick={() => {
                    ...
                    document.body.removeChild(a)
                    URL.revokeObjectURL(url)
                  }} className="px-3 py-2 bg-slate-900 text-white rounded-lg text-sm flex items-center"><Download className="w-4 h-4 mr-2"/>Dışa Aktar</button>
                </div>
              </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-slate-200">
                      ...
                      <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase">Alıcı</th>
                      ...
                      return (
                        <>
                          {pageTx.map((tx:any, idx:number) => (
                            <tr key={`${idx}-${tx.createdAt||tx.date||''}`} className="hover:bg-slate-50">
                              <td className="px-4 py-3 text-sm text-slate-700">{formatDateDDMMYYYY(tx.createdAt || tx.date)}</td>
                              <td className="px-4 py-3 text-sm">
                                <span className={`px-2 py-1 rounded text-xs font-medium ${tx.type==='credit' ? 'bg-green-100 text-green-700' : tx.type==='debit' ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-700'}`}>{tx.type==='credit'?'Yükleme': tx.type==='debit' ? 'Harcama' : 'Transfer'}</span>
                              </td>
                              <td className="px-4 py-3 text-sm text-slate-700">{(String(tx.type||'').toLowerCase()==='transfer') ? getRecipient(tx) : '-'}</td>
                              ...
                              <td className="px-4 py-3 text-sm">
                                {(() => {
                                  ...
                                  const badgeClass = s.includes('complete') || s==='completed' || s==='success' ? 'bg-blue-100 text-blue-700' : s.includes('pending') || s==='awaiting' ? 'bg-yellow-100 text-yellow-700' : s.includes('cancel') ? 'bg-slate-200 text-slate-700' : s.includes('reject') || s==='failed' || s.includes('fail') ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-700'
                          ...
                          <tr>
                            <td colSpan={5} className="px-4 py-3">
                              <div className="flex items-center justify-between">
                                <div className="text-sm text-slate-600">Toplam {total} işlem</div>
                                <div className="flex items-center gap-2">
                                  <button disabled={txPage<=1} onClick={()=> setTxPage(p=> Math.max(1, p-1))} className="px-3 py-1.5 border border-slate-200 rounded-lg disabled:opacity-50">Önceki</button>
                                  <span className="text-sm text-slate-700">{txPage}/{Math.max(1, Math.ceil(total/txPageSize))}</span>
                                  <button disabled={txPage>=Math.ceil(total/txPageSize)} onClick={()=> setTxPage(p=> p+1)} className="px-3 py-1.5 border border-slate-200 rounded-lg disabled:opacity-50">Sonraki</button>
                                  <select value={txPageSize} onChange={(e)=>{ setTxPageSize(parseInt(e.target.value)); setTxPage(1) }} className="ml-2 px-2 py-1 border border-slate-200 rounded-lg text-sm">
                                    {[10,20,50].map(n => (<option key={n} value={n}>{n}/sayfa</option>))}
                                  </select>
                                </div>
                              </div>
