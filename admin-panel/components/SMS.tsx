  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-slate-800">SMS Pazarlama</h2>
          <p className="text-slate-500 mt-1">SMS şablonları ve kampanya yönetimi</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl shadow-sm p-5 border border-slate-200">
          <p className="text-slate-500 text-sm mb-1">Teslimat Oranı</p>
          <p className="text-3xl font-bold text-green-600">0%</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm p-5 border border-slate-200">
          <p className="text-slate-500 text-sm mb-1">Aktif Şablon</p>
                ...
                <div className="flex-1 max-w-md">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500"
                    />
                  </div>
                </div>
                <button 
              ...
              <div className="space-y-3">
                {templates.map((template, index) => (
                  <motion.div
                    ...
                    className="flex items-start justify-between p-4 bg-white border border-slate-200 rounded-xl hover:shadow-md transition-all"
                  >
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-2">
                        ...
                        <span className="text-xs text-slate-500">{template.charCount} karakter</span>
                      </div>
                      <p className="text-sm text-slate-600 mb-2">{template.message}</p>
                    ...
                    <div className="flex items-center space-x-2 ml-4">
                      <button className="p-2 hover:bg-green-50 rounded-lg transition-colors" title="Düzenle">
                        <Edit className="w-5 h-5 text-green-600" />
                      </button>
                      <button className="p-2 hover:bg-blue-50 rounded-lg transition-colors" title="Kopyala">
              ...
              <div className="space-y-3">
                {campaigns.map((campaign, index) => (
                  <motion.div
                    ...
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: index * 0.05 }}
                    className="p-5 bg-white border border-slate-200 rounded-xl hover:shadow-md transition-all"
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-slate-600 mb-3">{campaign.message}</p>
                        <div className="grid grid-cols-4 gap-4">
                          <div>
                            <p className="text-xs text-slate-500 mb-1">Alıcı</p>
                            <p className="font-bold text-slate-800">{campaign.recipients.toLocaleString()}</p>
                          </div>
                          <div>
                            <p className="text-xs text-slate-500 mb-1">Gönderilen</p>
                          ...
                          <div>
                            ...
                            <p className="font-bold text-slate-800">{campaign.date}</p>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            >
              <div className="p-6 border-b border-slate-200 flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-slate-800">Yeni SMS Şablonu</h3>
                    <p className="text-sm text-slate-500">SMS içeriğinizi oluşturun</p>
                  </div>
                </div>
                <button
                  onClick={() => setShowCreateModal(false)}
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500"
                    />
                  </div>
                  <div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="block text-sm font-medium text-slate-700">SMS İçeriği *</label>
                    <span className="text-xs text-slate-500">{templateMessage.length} / 160 karakter</span>
                  </div>
                  <textarea
                    ...
                    rows={4}
                    maxLength={160}
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 resize-none"
                  />
                </div>

                <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-4 border border-green-200">
                  <p className="text-sm font-medium text-slate-700 mb-2">Değişken Ekle:</p>
                  <div className="flex flex-wrap gap-2">
                    <button
                      onClick={() => insertVariable('AD')}
                      className="px-3 py-1.5 bg-white border border-green-200 rounded-lg text-xs font-medium text-slate-700 hover:bg-green-50 transition-colors"
                    >
                      Ad
                    </button>
                    <button
